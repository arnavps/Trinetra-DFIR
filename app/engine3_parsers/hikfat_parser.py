"""Hikvision HIKFAT parser — locates end-of-drive index tables, maps channel/time allocation. Byte offsets live in a separate constants block, seeded from published literature and corrected against real drives as they're imaged."""
...
class HikFatParser(FileSystemParser):
    """Hikvision HIKFAT filesystem parser implementation."""
    ...
    def parse(self, image_path: str) -> VirtualFileSystem:
        ...
        with ImageReader(image_path) as f:
            ...
            if file_size < len(const.HIKFAT_SUPERBLOCK_MAGIC):
                raise ValueError("Image file too small to contain Hikvision superblock.")

            f.seek(const.HIKFAT_SUPERBLOCK_OFFSET)
            superblock_magic = f.read(len(const.HIKFAT_SUPERBLOCK_MAGIC))
            if superblock_magic != const.HIKFAT_SUPERBLOCK_MAGIC:
                raise ValueError(f"Invalid HIKFAT superblock signature: {superblock_magic}")

            # 2. Locate Index Table
            index_offset = file_size - const.HIKFAT_INDEX_TABLE_OFFSET_FROM_END
            if index_offset < 0:
                raise ValueError("Image file size smaller than HIKFAT index table offset.")

            f.seek(index_offset)
            index_magic = f.read(len(const.HIKFAT_INDEX_MAGIC))
            if index_magic != const.HIKFAT_INDEX_MAGIC:
                raise ValueError(f"Invalid HIKFAT index table signature: {index_magic}")
            ...
            for idx in range(record_count):
                record_bytes = f.read(const.HIKFAT_INDEX_RECORD_SIZE)
                if len(record_bytes) < const.HIKFAT_INDEX_RECORD_SIZE:
                    break

                channel_id, start_ts, end_ts, start_sec, sec_count, fsize = struct.unpack(
                    const.HIKFAT_INDEX_RECORD_STRUCT, record_bytes
                )
                ...
                entry = ExtractedFileEntry(
                    file_id=f"HIK_CH{channel_id}_{idx+1:04d}",
            ...
            return VirtualFileSystem(oem="Hikvision", channels=channels, files=files)
