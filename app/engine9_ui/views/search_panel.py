...
def query_annotations(
    ...
    """
    Queries detection and face annotation records from engine7_case_db database
    filtered by class, camera channel, and time range.
    """
    ...
    try:
        ...
        # Query face detections if class_filter is "All" or "Face"
...
class SearchPanelWidget(QWidget):
    ...
    def perform_search(self):
        ...
        for row_idx, item in enumerate(results):
            ...
            self.table.setItem(row_idx, 2, QTableWidgetItem(str(item["class_name"])))
            ...
            reid_text = item["reid_label"] if item["reid_label"] else ("-" if item["class_name"] not in ("person", "car") else INVESTIGATIVE_LEAD_LABEL)
