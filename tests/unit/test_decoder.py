...
def test_no_mp4_mkv_in_workspace():
    ...
    primary_mp4 = [f for f in all_mp4 if "demo_case" not in f and "derivatives" not in f and "docs" not in f and not os.path.basename(f).startswith("export_")]
    primary_mkv = [f for f in all_mkv if "demo_case" not in f and "derivatives" not in f and "docs" not in f and not os.path.basename(f).startswith("export_")]
